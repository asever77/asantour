...
(function ($, win, doc, undefined) {
    ...
    (function () {
        var width = document.documentElement.offsetWidth,
            ...
            sizeMode, colClass = width > devsize[5] ? 'col12' : width > devsize[8] ? 'col8' : 'col4',
    ...
    (function () {
        ...
        browser.app = ua.indexOf('appname') > -1 ? !0 : !1;
        ...
        if (ie) {
            ...
            browser.oldie = !1;
            browser.ie9 = !1;
            (9 > ie) ? browser.oldie = !0: (9 == ie) ? browser.ie9 = !0 : '';
        ...
        $('html').addClass(browser.os).addClass(browser.chrome ? 'chrome' : browser.firefox ? 'firefox' : browser.opera ? 'opera' : browser.safari ? 'safari' : browser.ie ? 'ie ie' + browser.ie : '').addClass(browser.ie && 8 > browser.ie ? 'oldie' : '').addClass(browser.ios ? "ios" : browser.android ? "android" : '').addClass(browser.mobile ? 'ui-m' : 'ui-d').addClass(browser.app ? 'ui-a' : '')
    })();
    ...
    function createUiAjax(opt) {
        ...
        var opt = opt === undefined ? {} : opt,
            opt = $.extend(!0, {}, win[global].uiAjax.option, opt),
            $id = $('#' + opt.id),
            callback = opt.callback === undefined ? !1 : opt.callback,
            errorCallback = opt.errorCallback === undefined ? !1 : opt.errorCallback;
        $.ajax({
            type: opt.type,
            url: opt.url,
            cache: !1,
    ...
    function createUiScroll(opt) {
        ...
        var opt = $.extend(!0, {}, win[global].uiScroll.option, opt),
            v = opt.value,
            s = opt.speed,
            c = opt.callback,
            p = opt.ps,
            ...
            $target = opt.target === !1 ? $('html, body') : opt.target;
        if (p === 'top') {
            $target.stop().animate({
                scrollTop: v
            }, {
    ...
    function createUiFocusTab(opt) {
        ...
        var opt = opt === undefined ? {} : opt,
            opt = $.extend(!0, {}, win[global].uiFocusTab.option, opt),
            $focus = $(opt.selector),
            ...
            timer;
        if (!!$item.length) {
            $item.eq(0).addClass('ui-fctab-s').attr('tabindex', 0).attr('holds', !0);
            $item.eq(-1).addClass('ui-fctab-e').attr('tabindex', 0).attr('holde', !0)
        } else {
    ...
    function createUiPopup(opt) {
        var opt = opt === undefined ? {} : opt,
            opt = $.extend(!0, {}, win[global].uiPopup.option, opt),
            specs;
        if (opt.align === 'center') {
    ...
    function creaeteUiCookieSet(opt) {
        ...
        if (opt.term) {
            expdate = new Date();
            expdate.setTime(expdate.getTime() + opt.term * 1000 * 60 * 60 * 24);
    ...
    function createUiAccordion(opt) {
        ...
        var opt = $.extend(!0, {}, win[global].uiTab.option, opt),
            id = opt.id,
            current = opt.current,
            callback = opt.callback,
        ...
        for (i = 0; i < len; i++) {
            ...
            $accobtn.data('selected', !1).attr('data-n', i).attr('data-len', len).attr('aria-expanded', !1).attr('aria-controls', $accopln.attr('id')).removeClass('selected').find('.ui-acco-txt').text('열기');
            $accopln.attr('data-n', i).attr('data-len', len).attr('aria-labelledby', $accobtn.attr('id')).attr('aria-hidden', !0).hide();
            i === 0 ? $accobtn.attr('acco-first', !0) : '';
            i === len - 1 ? $accobtn.attr('acco-last', !0) : ''
        }
        current !== null ? win[global].uiAccordionToggle({
            id: id,
            current: current,
            motion: !1
        }) : '';
        ...
        function evtKeys(e) {
            ...
            function upLeftKey(e) {
                e.preventDefault();
                !$this.attr('acco-first') ? $('#' + id + '-btn' + (n - 1)).focus() : $('#' + id + '-btn' + (m - 1)).focus()
            }

            function downRightKey(e) {
                e.preventDefault();
                !$this.attr('acco-last') ? $('#' + id + '-btn' + (n + 1)).focus() : $('#' + id + '-btn0').focus()
            }
    ...
    function createUiAccordionToggle(opt) {
        ...
        var id = opt.id,
            ...
            motion = opt.motion === undefined ? !0 : opt.motion,
            autoclose = dataOpt.close,
            allshow = opt.allshow,
            allhide = opt.allhide,
            open = null,
            $wrap = $acco.children('.ui-acco-wrap'),
            $pnl, $tit, $btn, len = $wrap.length,
            speed = 200,
            i, c = 0;
        (motion === !1) ? speed = 0: speed = 200;
        if (current !== 'all') {
        ...
        function act(v) {
            var isDown = v === 'down',
                ...
                txt = isDown ? '닫기' : '열기';
            open = isDown ? !0 : !1;
            if (autoclose === !0 && isDown) {
                $wrap.each(function (i) {
                    $wrap.eq(i).find('> .ui-acco-tit .ui-acco-btn').data('selected', !1).removeClass('selected').attr('aria-expanded', !1).find('.ui-acco-txt').text('열기');
                    $wrap.eq(i).find('> .ui-acco-pnl').attr('aria-hidden', !0).stop().slideUp(speed)
                })
            }
            if (current === 'all') {
                $wrap.each(function (i) {
    ...
    function createUiDropdown(opt) {
        ...
        var opt = $.extend(!0, {}, win[global].uiDropdown.option, opt),
            id = opt.id,
            eff = opt.eff,
            ps = opt.ps,
            hold = opt.hold,
            ...
            $btn = $('#' + id),
            $pnl = $('[data-id="' + id + '"]');
        $btn.attr('aria-expanded', !1).data('opt', {
            id: id,
            eff: eff,
            ps: ps,
            hold: hold,
        ...
        $pnl.attr('aria-hidden', !0).attr('aria-labelledby', id).addClass(ps).data('opt', {
            id: id,
            eff: eff,
            ps: ps,
            hold: hold,
        ...
        $(doc).off('click.dropdownclose').on('click.dropdownclose', '.ui-drop-close', function (e) {
    ...
    function createUiDropdownToggle(opt) {
        ...
        function pnlShow() {
            var org_t, org_l, drop_inner = $btn.closest('.ui-drop-pnl').data('id');
            if (_close) {
                if (!!drop_inner) {
                    $('.ui-drop').not('#' + drop_inner).attr('aria-expanded', !1);
                    $('.ui-drop-pnl').not('[data-id="' + drop_inner + '"]').attr('aria-hidden', !0).attr('tabindex', -1).removeAttr('style')
                } else {
                    win[global].uiDropdownHide()
                }
            }
            $btn.attr('aria-expanded', !0);
            $pnl.attr('aria-hidden', !1).attr('tabindex', 0);
            hold ? win[global].uiFocusTab({
            ...
            switch (ps) {
                ...
                case 'bc':
                    $pnl.css({
                        top: btn_t + btn_h,
                        left: btn_l - ((pnl_w - btn_w) / 2)
                    });
                    break;
                case 'br':
                    $pnl.css({
                        top: btn_t + btn_h,
                        left: btn_l - (pnl_w - btn_w)
                    });
                    break;
                case 'tl':
                    $pnl.css({
                ...
                case 'rm':
                    $pnl.css({
                        top: btn_t - ((pnl_h - btn_h) / 2),
                        left: btn_l + btn_w
                    });
                    break;
                case 'rb':
                    $pnl.css({
                        top: btn_t - (pnl_h - btn_h),
                        left: btn_l + btn_w
                    });
                    break;
                case 'lt':
            ...
            org_t = parseInt($pnl.css('top')), org_l = parseInt($pnl.css('left'));
            switch (eff) {
                case 'base':
                    $pnl.stop().show(0);
                    break;
                case 'fade':
                    $pnl.stop().fadeIn(eff_speed);
                    break;
                case 'st':
                    $pnl.css({
                        top: org_t - eff_ps,
            ...
            setTimeout(function () {
                $('body').data('dropdownOpened', !0).addClass('dropdownOpened')
            }, 0)
        }
        ...
        function pnlHide() {
            ...
            if ($pnl.closest('.ui-drop-box').length < 1) {
                $('body').data('dropdownOpened', !1).removeClass('dropdownOpened')
            }
            $btn.attr('aria-expanded', !1).focus();
            $pnl.attr('aria-hidden', !0).attr('tabindex', -1);
            switch (eff) {
                case 'base':
                    $pnl.stop().hide(0, pnlHideEnd);
                    break;
                case 'fade':
                    $pnl.stop().fadeout(eff_speed, pnlHideEnd);
                    break;
                case 'st':
                    $pnl.stop().animate({
                        top: org_t - eff_ps,
                        opacity: 0
                    }, eff_speed, pnlHideEnd);
                    break;
                case 'sb':
                    $pnl.stop().animate({
                        top: org_t + eff_ps,
    ...
    function createUiDropdownHide() {
        $('body').data('dropdownOpened', !1).removeClass('dropdownOpened');
        $('.ui-drop').attr('aria-expanded', !1);
        $('.ui-drop-pnl').attr('aria-hidden', !0).attr('tabindex', -1).removeAttr('style')
    }
    win[global] = uiNameSpace(namespace, {
    ...
    function createUiDatePicker(opt) {
        var opt = $.extend(!0, {}, win[global].uiDatePicker.option, opt),
        ...
        function writeInputDateValue(calendarEl, obj) {
            ...
            calendarEl.shortDate ? d = toDDMMYYY(d) : '';
            $("#" + id).val(d);
        ...
        function buildCalendar(date, calendarEl, v) {
            ...
            _calendarHtml += year < _minDay[0] || year == _minDay[0] && dateMonths[month] <= _minDay[1] ? '<button type="button" class="ui-datepicker-prev disabled" disabled>' : '<button type="button" class="ui-datepicker-prev">';
            _calendarHtml += '<span>이전 ' + dateMonths[(month === 0) ? 11 : month - 1] + ' 월로 이동</span></button>';
            _calendarHtml += year > _maxDay[0] || year == _maxDay[0] && dateMonths[month] >= _maxDay[1] ? '<button type="button" class="ui-datepicker-next disabled" disabled>' : '<button type="button" class="ui-datepicker-next">';
            _calendarHtml += '<span>다음 ' + dateMonths[(month == 11) ? 0 : month + 1] + ' 월로 이동</span></button>';
            _calendarHtml += '</div>';
            _calendarHtml += '<div class="today"><button type="button" class="today" data-day=' + textDate(dateToday.getDate(), dateToday.getMonth() + 1, dateToday.getFullYear(), !0) + '><span>오늘 - ' + textDate(dateToday.getDate(), dateToday.getMonth() + 1, dateToday.getFullYear(), !0) + ' 이동</span></button></div>';
            ...
            for (var dayCounter = 1; dayCounter <= daysInMonth; dayCounter++) {
                ...
                if ((year < _minDay[0]) || (year == _minDay[0] && dateMonths[month] < _minDay[1]) || (year == _minDay[0] && dateMonths[month] == _minDay[1] && dayCounter < _minDay[2])) {
                    _calendarHtml += '<span title="' + textDate(dayCounter, mm, year, !0) + '">' + partsAdd0(dayCounter) + '</span></td>'
                    ...
                    _calendarHtml += '<button type="button" title="' + textDate(dayCounter, mm, year, !0) + '" data-day="' + textDate(dayCounter, mm, year, !1) + '" value="' + dayCounter + '">' + partsAdd0(dayCounter) + '</button></td>'
                }
                week_day++
            }
        ...
        $datepicker.each(function () {
            ...
            win[global].uiDropdown({
                id: $(this).attr('id'),
                eff: 'st',
                ps: 'bc'
            });
            $.browser.mobile ? $('#' + $btn.data('inp')).prop('readonly', !0).attr('aria-hidden', !0) : ''
        });
        $datepicker.find('.ui-datepicker-btn').off('focus.uidpbtn mouseover.uidpbtn').on('focus.uidpbtn mouseover.uidpbtn', function () {
            ...
            $this.attr('aria-expanded') === 'false' || $this.attr('aria-expanded') === undefined ? win[global].uiDropdown({
                id: dropid,
                eff: _ef,
                ps: _ps
            }) : ''
        ...
        $datepicker.find('.ui-datepicker-btn').off('click.uidpbtn').on('click.uidpbtn', function () {
            ...
            $datepicker.find('.tbl-datepicker button[data-day="' + $('#' + inputId).val() + '"]').addClass('selected').attr('aria-selected', !0)
        })
    }
    win[global] = uiNameSpace(namespace, {
    ...
    function createUiModal(opt) {
        ...
        var opt = $.extend(!0, {}, win[global].uiModal.option, opt),
            id = opt.id,
        ...
        if (!src) {
            ...
            !$modal.length ? win[global].uiAjax({
                id: 'baseLayer',
                url: src,
                page: !0,
                add: !0,
    ...
    function modalOpen(opt) {
        var id = opt.id,
            ...
            overW = winW <= w,
            overH = winH <= h,
            iw, ih, is_m = $.browser.mobile,
            is_head = !!$modalHead.length,
            is_foot = !!$modalFoot.length,
            is_full_h, is_full_w, is_iframe, mh_h, mf_h, timer, modal_n = 0,
            re_num = 0,
            re_timer, modalbackdrop, words = opt.words,
        ...
        $('#baseWrap').attr('aria-hidden', !0);
        $modal.data('opt', opt).data('scrolltop', $(win).scrollTop()).attr('opened', !0).attr('aria-hidden', !1).attr('aria-labelledby', id + '-tit').find('.ui-modal-tit').attr('id', id + '-tit');
        ...
        if (modal_n === 1) {
            ...
            $modal.siblings('.ui-modal').attr('aria-hidden', !0);
        ...
        function reModal(v) {
            ...
            if (is_full_h) {
                ...
                $modal.css('height', h);
                words ? $modalCont.css('height', 'auto') : $modalCont.css('height', Math.ceil(h - (mh_h + mf_h)) + 'px')
            }
            if (is_iframe) {
            ...
            if (!!v.timer) {
                ...
                if (!v.resize) {
                    !!iname ? $modal.data('iframeload') === undefined ? reExe('b') : exe() : $modalCont.outerHeight() < 10 ? reExe('a') : exe()
                }
            }
            ...
            function showEnd() {
                ...
                win[global].callback.modal(opt.id);
                if (is_iframe) {
                    frames[opt.iname].$('.wrap-iframe').mCustomScrollbar({
                        scrollButton: {
                            enable: !0
                        }
                    })
    ...
    function createUiModalClose(opt) {
        ...
        var $body = $('body'),
            ...
            opt = $.extend(!0, {}, $modal.data('opt'), opt),
        ...
        if (modal_n < 2) {
            switch (ps) {
                case 'top':
                    $modal.attr('aria-hidden', !0).stop().animate({
                        opacity: 0,
                        top: h * -1
            ...
            $modal.attr('aria-hidden', !0).stop().animate({
                ...
                layRemove === !0 ? $modal.remove() : $modal.removeAttr('style').removeClass('scrollpop').removeAttr('opened');
                autofocus ? $(endfocus).attr('tabindex', 0).focus() : '';
                $('.ui-modal[n="' + z + '"]').attr('aria-hidden', !1);
        ...
        function closed() {
            layRemove === !0 ? $modal.remove() : $lay.removeAttr('style').removeAttr('opened');
    ...
    function creaeteUiCookieModal(opt) {
        ...
        function open() {
            win[global].uiModal({
                id: opt.id,
                full: opt.full === undefined ? !1 : opt.full,
                link: opt.link === undefined ? !1 : opt.link
            })
        }
    }

    function creaeteUiCookieModalClose(opt) {
        $('#' + opt.cookiename).prop('checked') ? win[global].uiCookieSet({
            name: opt.cookiename,
            value: !0,
            term: 365
        }) : '';
    ...
    function createUiSelect(opt) {
        var opt = opt === undefined ? {} : opt,
            opt = $.extend(!0, {}, win[global].uiSelect.option, opt),
            current = opt.current,
            id = opt.id,
            is_id = id === !1 ? !1 : !0,
            ...
            _val = '',
            _txt = '',
            $sel, $sel_current, $opt, $opt_current, sel_id, list_id, opt_id, opt_id_selected, sel_n, sel_tit, sel_dis, opt_len, id_opt, id_optname, idx, timer_opt, timer, _option_wrap = '';
        ...
        for (i = 0; i < len; i++) {
            ...
            for (j = 0; j < opt_len; j++) {
                ...
                if (current !== null) {
                    _selected = current === j ? $opt_current.prop('selected', !0) : $opt_current.prop('selected', !1)
                } else {
                ...
                _disabled ? _selected ? _option_wrap += '<button type="button" role="option" id="' + opt_id + '_' + j + '" class="ui-select-opt disabled selected" value="' + $opt_current.val() + '" disabled tabindex="-1">' : _option_wrap += '<button type="button" role="option" id="' + opt_id + '_' + j + '" class="ui-select-opt disabled" value="' + $opt_current.val() + '" disabled tabindex="-1">' : _selected ? _option_wrap += '<button type="button" role="option" id="' + opt_id + '_' + j + '" class="ui-select-opt selected" value="' + $opt_current.val() + '" tabindex="-1">' : _option_wrap += '<button type="button" role="option" id="' + opt_id + '_' + j + '" class="ui-select-opt" value="' + $opt_current.val() + '" tabindex="-1">';
            ...
            $sel.addClass('off').attr('aria-hidden', !0).attr('tabindex', -1);
            $sel_current.append(_option_wrap);
            sel_dis ? $sel_current.find('.ui-select-btn').prop('disabled', !0).addClass('disabled') : '';
            _option_wrap = ''
        }
        $('.ui-select-btn').off('click.ui keydown.ui mouseover.ui focus.ui blur.ui').on({
        ...
        $('.ui-select select').off('change.ui').on({
        ...
        function optScroll($wrap, n_top, wrap_h, key) {
            if (key === 'up') {
                n_top < 0 ? $wrap.stop().animate({
                    'scrollTop': $wrap.scrollTop() - wrap_h
                }) : n_top > wrap_h ? $wrap.stop().animate({
                    'scrollTop': n_top
                }) : ''
            } else {
                n_top >= wrap_h ? $wrap.stop().animate({
        ...
        function optOpen(t) {
            ...
            if (!_$sel.data('expanded')) {
                _$sel.data('expanded', !0).attr('aria-expanded', !0);
                _$uisel.addClass('on');
                _$wrap.addClass('on ' + clsname).attr('aria-hidden', !1);
                _$opts.find('.ui-select-opt').eq(_$uisel.find(':checked').index());
                win[global].uiScroll({
                    target: _$wrap,
                    value: Number(opth * _$uisel.find(':checked').index()),
                    speed: 0
                })
            }
        }
        ...
        function optClose() {
            ...
            $btn.data('expanded', !1).attr('aria-expanded', !1);
            $select.removeClass('on');
            $wrap.removeClass('on top bottom').attr('aria-hidden', !0)
        }
    }
    ...
    function createUiSelectAct(opt) {
        var id = typeof opt.id === 'string' ? opt.id : opt.id.attr('id'),
            ...
            callback = opt.callback === undefined ? !1 : opt.callback,
            current = opt.current,
            org = opt.original === undefined ? !1 : opt.original;
        !org ? $uisel.find('option').prop('selected', !1).eq(current).prop('selected', !0).change() : '';
        $uisel.find('.ui-select-btn').val($opt.eq(current).text());
        $opt_.removeClass('selected').eq(current).addClass('selected');
        callback ? callback({
            id: id,
            current: current,
            val: $opt.eq(current).val()
        }) : ''
    }
    win[global] = uiNameSpace(namespace, {
        uiTab: function (opt) {
    ...
    function createUiTab(opt) {
        var opt = opt === undefined ? {} : opt,
            opt = $.extend(!0, {}, win[global].uiTab.option, opt),
            id = opt.id,
        ...
        for (i = 0; i < len; i++) {
            ...
            is_current ? _$btn.attr('aria-selected', !0).addClass('selected') : _$btn.attr('aria-selected', !1).removeClass('selected');
            ps_l.push(Math.ceil(_$btn.position().left));
            i === 0 ? _$btn.attr('tab-first', !0) : '';
            i === len - 1 ? _$btn.attr('tab-last', !0) : ''
        }
        callback ? callback(opt) : '';
        ...
        function evtKeys(e) {
            ...
            function upLeftKey(e) {
                e.preventDefault();
                !$this.attr('tab-first') ? win[global].uiTabAct({
                    id: id,
                    current: n - 1
                }) : win[global].uiTabAct({
                    id: id,
            ...
            function downRightKey(e) {
                e.preventDefault();
                !$this.attr('tab-last') ? win[global].uiTabAct({
                    id: id,
                    current: n + 1
                }) : win[global].uiTabAct({
                    id: id,
    ...
    function createUiTabAct(opt) {
        var id = opt.id,
            ...
            $pnl = $pnls.children('.ui-tab-pnl'),
            ps_l = $btn.data('psl'),
            opt = $.extend(!0, {}, $tab.data('opt'), opt),
            current = isNaN(opt.current) ? 0 : opt.current,
            unres = opt.unres,
            callback = opt.callback;
        $btn.attr('aria-selected', !1).attr('tabindex', -1).removeClass('selected').eq(current).attr('aria-selected', !0).removeAttr('tabindex').addClass('selected').focus();
        win[global].uiScroll({
            value: ps_l[current],
            target: $btns,
            speed: 200,
    ...
    function createUiTooltip(opt) {
        var opt = opt === undefined ? {} : opt,
            opt = $.extend(!0, {}, win[global].uiTblScroll.option, opt),
            $btn = $('.ui-tooltip-btn'),
            $tip = opt.id ? typeof opt.id === 'string' ? $('#' + opt.id) : opt.id : !1,
            visible = opt.visible,
            id = opt.id ? $tip.attr('id') : '',
            sp = 10,
            ps = opt.ps,
            off_t, off_l, w, h, bw, bh, st, sl, timer, class_ps = 'ps-ct ps-cb ps-lt ps-lb ps-rt ps-rb';
        if (visible !== null) {
        ...
        function tooltipSet(v) {
            ...
            st = $(doc).scrollTop();
            sl = $(doc).scrollLeft();
            tooltipShow(off_t, off_l, w, h, bw, bh, st, sl, id, !1)
        }

        function tooltipHide() {
            $('.ui-tooltip').removeAttr('style').attr('aria-hidden', !0).removeClass(class_ps)
        }
        ...
        function tooltipShow(off_t, off_l, w, h, bw, bh, st, sl, id) {
            var $id = $('#' + id),
                pst = (bh / 2 > (off_t - st) + (h / 2)) ? !0 : !1,
                psl = (bw / 2 > (off_l - sl) + (w / 2)) ? !0 : !1,
                tw = $id.outerWidth(),
                th = $id.outerHeight(),
            ...
            $id.addClass(cursorCls).attr('aria-hidden', !1).css({
                display: 'block'
            }).css({
    ...
    function createUiFloating(opt) {
        var opt = opt === undefined ? {} : opt,
            opt = $.extend(!0, {}, win[global].uiFloating.option, opt),
            id = opt.id,
            ps = opt.ps,
            add = opt.add,
            fix = opt.fix,
        ...
        function act() {
            ...
            if (ps === 'top') {
                if (fix === !0) {
                    if (tt - lh <= st) {
                        $id.removeClass(c).data('fixtop', !1);
    ...
    function createUiCodinglist(opt) {
        ...
        function callback(v) {
            ...
            var len = dataExecel.list.length,
                i = 0,
                state, date, end, pub, id, idm, pop, modal, tab, memo, overl, dev, d1, d2, d3, d4, d5, d6, d7, d8, r1, r2, r3, r4, d1_, d2_, d3_, d4_, d5_, d6_, d7_, d8_, c1, c2, c3, c4, c5, c6, c7, c8, endsum = 0,
                delsum = 0,
                tstsum = 0,
                ingsum = 0,
                watsum = 0,
            ...
            for (i = 0; i < len; i++) {
                ...
                (dataExecel.list[i].d1 === '' && dataExecel.list[i].d2 === '' && dataExecel.list[i].d3 === '' && dataExecel.list[i].d4 === '') ? (d4 !== '') ? d5_ = dataExecel.list[i - 1 < 0 ? 0 : i].d5: d5 = d5_: (!!dataExecel.list[i - 1 < 0 ? 0 : i].d5) ? d5_ = dataExecel.list[i - 1 < 0 ? 0 : i].d5 : d5_ = '';
                (dataExecel.list[i].d1 === '' && dataExecel.list[i].d2 === '' && dataExecel.list[i].d3 === '' && dataExecel.list[i].d4 === '' && dataExecel.list[i].d5 === '') ? (d4 !== '') ? d6_ = dataExecel.list[i - 1 < 0 ? 0 : i].d6: d6 = d6_: (!!dataExecel.list[i - 1 < 0 ? 0 : i].d6) ? d6_ = dataExecel.list[i - 1 < 0 ? 0 : i].d6 : d6_ = '';
                (dataExecel.list[i].d1 === '' && dataExecel.list[i].d2 === '' && dataExecel.list[i].d3 === '' && dataExecel.list[i].d4 === '' && dataExecel.list[i].d5 === '' && dataExecel.list[i].d6 === '') ? (d4 !== '') ? d7_ = dataExecel.list[i - 1 < 0 ? 0 : i].d7: d7 = d7_: (!!dataExecel.list[i - 1 < 0 ? 0 : i].d7) ? d7_ = dataExecel.list[i - 1 < 0 ? 0 : i].d7 : d7_ = '';
                ...
                if (state !== '제외' && i === 0) {
                    ...
                    if (!modalIs) {
                        table += (id !== '') ? (overl !== '') ? '<td class="id ico_pg"><span><a href="' + root + '' + overl + '.html" target="' + target + '" class="' + popcls + '">' + overl + '</a></span><span class="overl">' + id + '</span></td>' : '<td class="id ico_pg"><span><a href="' + root + '' + id + '.html" target="' + target + '" class="' + popcls + '">' + id + '</a></span></td>' : '<td class="id "><span></span></td>'
                    } else {
                        table += (id !== '') ? (overl !== '') ? '<td class="id ico_pg"><span><button type="button" onclick="$plugins.uiModal({ id:\'modal_' + overl + '\', src:\'' + root + '' + overl + '.html\'});">' + overl + '</button></span><span class="overl">' + id + '</span></td>' : '<td class="id ico_pg"><span><button type="button" onclick="$plugins.uiModal({ id:\'modal_' + id + '\', src:\'' + root + '' + id + '.html\'});">' + id + '</button></span></td>' : '<td class="id "><span></span></td>'
            ...
            if (!$('.ui-codinglist-info .total').data('data')) {
                $('.ui-codinglist-info .total').data('data', !0).text(len - delsum - 1);
            ...
            selectoption('uiCLDate', ctg_date, !0);
            ...
            function selectoption(id, optarray, v) {
                ...
                if (!$sel.data('data')) {
                    ...
                    $sel.data('data', !0).append(optionHtml)
                }
            }
            ...
            function selectAct() {
                $('.ui-codinglist-sel select').off('change.cdlist').on('change.cdlist', function () {
                    ...
                    if (v === '0') {
                        ...
                        $this.siblings().find('option:eq(0)').prop('selected', !0);
                        $sel.each(function (i) {
                            v === 'all' ? $sel.closest('tr').removeClass('hidden') : v !== $sel.find('span').eq(i).text() ? $(this).closest('tr').addClass('hidden') : $(this).closest('tr').removeClass('hidden')
                        })
                    }
                })
            }
            $('.ui-codinglist-sel button').off('click.cdlist').on('click.cdlist', function (e) {
                ...
                $('.ui-codinglist-sel select').find('option:eq(0)').prop('selected', !0)
            })
        }
    }
    win[global] = uiNameSpace(namespace, {
    ...
    function createUiFileUpload(opt) {
        ...
        var base = {};
        base.id = $('#' + opt.id);
        base.multi = opt.multi === undefined ? !1 : opt.multi;
        base.accept = opt.accept === undefined ? '' : 'accept="' + opt.accept + '"';
        base.callback = opt.callback === undefined ? !1 : opt.callback;
        base.n = 0;
        ...
        function fileChange(base) {
            ...
            if (!!base.v && !base.file.val()) {
                base.txt.remove();
                base.del.remove();
                base.id.data('single', !1)
            }

            function act(v) {
                v === 'single' ? base.id.data('single', !0) : '';
                v === 'multi' ? base.id.data('multi', !0) : '';
    ...
    function createUiPhotoSlide(opt) {
        ...
        function init() {
            ...
            $slide.find('.owl-item').eq(0).find('.ui-photoslide-btn').data('first', !0);
            if (!!$wrap.data('n')) {
            ...
            !!imgsrc2 ? imgChange(imgsrc2, imgalt, current) : imgChange(imgsrc, imgalt, current);
            $btn.on('click', function () {
                var $this = $(this),
                    $this_img = $this.find('img');
                $this.closest('.ui-photoslide-slide').find('.ui-photoslide-btn').removeClass('selected');
                $this.addClass('selected');
                !!$this_img.data('src') ? imgChange($this_img.data('src'), $this_img.attr('alt'), $this.index()) : imgChange($this_img.attr('src'), $this_img.attr('alt'), $this.index())
            });

            function imgChange(j, h, m) {
                $wrap.data('n', m);
                $img.attr('src', j)
                $txt.html(h)
            }
            ...
            function nextprev(n, id, items, s) {
                ...
                if (nn > (len)) {
                    ...
                    !!$a.data('first') ? '' : $('#' + id).find('.active .ui-photoslide-btn').eq(len).trigger('click')
                } else {
    ...
    function createUiScrolling(opt) {
        var opt = $.extend(!0, {}, win[global].uiScrolling.option, opt),
